...
os.system('rm /apps/code/dyzz_spider/tutorial/gotobt.json')
os.system('docker run --name scrapy --rm -v /apps/code/dyzz_spider/tutorial:/runtime/app aciobanu/scrapy scrapy crawl gotobt -o gotobt.json')

p = PyAria2(secret='prc_password')

with open('/apps/code/aria2/gotobt.txt','r') as a:
    content = a.read().splitlines()

    with open('/apps/code/dyzz_spider/tutorial/gotobt.json', 'r') as f:
        m = json.load(f)
        for _m in m:
            if _m['name'] in content:
                print('not ', _m['name'])
            if _m['name'] not in content:
                with open("gotobt.txt","a+") as out:
                    out.write(_m['name']+"\n")
                    a = p.addUri(uris=_m['magnet'], position= {"dir":"/downloads/qs"})
                    print(a)
